...
def remove_participant_from_game(game_id: str, game: Game, name_to_remove: str) -> str:
    ...
    - если получается самоназначение или что-то странное — пересчитываем всё целиком.
...
async def cmd_help(message: types.Message):
    text = (
        ...
        "5. Нажми кнопку «📝 Пожелание к подарку», чтобы задать или изменить пожелание.\n\n"
        "*Бот:*\n"
...
async def handle_wish_button(message: types.Message):
    ...
    if current_wish:
        await message.answer(
            f"Сейчас твоё пожелание к подарку:\n«{current_wish}»\n\n"
...
# ---------- Пожелания по подарку (участники, старая команда /wish — оставлена для совместимости) ----------
    ...
    if user_id not in user_games:
        ...
        if auto_bound:
            ...
            await message.answer(
                ...
                "Если хочешь, можешь нажать «📝 Пожелание к подарку», чтобы задать пожелание.",
                parse_mode="Markdown",
                reply_markup=kb,
            )
        else:
